...
int main(int argc, char *argv[])
{
	...
	model.set_optimizer(std::make_shared<PerceptronOptimizer<float>>());
	model.build();
	std::size_t nepochs = 100;
	...
	QObject::connect(controls, &ControlsWidget::startTraining,
			 [&](void) {
				 // Pass to the trainer the model and the epochs
				 ...
				 QObject::connect(trainer, &Trainer::updateMAE,
						  controls, &ControlsWidget::setMAE);
